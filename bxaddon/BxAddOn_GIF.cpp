...
{
	(void) opaque;
	BxASSERT("BxCore::ThirdParty<NSGIF003의 오동작입니다>", bitmap);
}
...
const byte* GIFToBMP(const byte* gif, const int length, int _out_ numpage)
{
	...
	const int _width = gifani.width;
	const int _height = gifani.height;
	const int BmpBytesPerPixel = 4;
	const int BmpRow = (BmpBytesPerPixel * _width + 3) & ~3;
	const int BmpPageHeight = _height * gifani.frame_count;
	byte* Bmp = (byte*) BxAlloc(4 + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + BmpRow * BmpPageHeight);
	Bmp[0] = 'B';
	Bmp[1] = 'x';
	Bmp[2] = 'B';
	Bmp[3] = 'M';
	BITMAPFILEHEADER* BmpFileHeader = (BITMAPFILEHEADER*)(Bmp + 4);
	BmpFileHeader->Size = 54 + BmpRow * BmpPageHeight;
	...
	for(int i = 0, iend = gifani.frame_count; i < iend; ++i)
	{
		...
		{
			for(int x = 0, s = 0; x + 3 < BmpRow; x += BmpBytesPerPixel, s += 4)
			{
				if(ImgFocus[s + 3] == 0)
				{
					BmpFocus[x + 0] = 0;
				...
				{
					BmpFocus[x + 0] = ImgFocus[s + 2];
					BmpFocus[x + 1] = ImgFocus[s + 1];
					BmpFocus[x + 2] = ImgFocus[s + 0];
					BmpFocus[x + 3] = ImgFocus[s + 3];
				}
			}
			BmpFocus += BmpRow;
			ImgFocus -= _width * 4;

...
const byte* JPGToBMP(const byte* jpg)
{
	...
	const int _width = input->inputAttributes[0];
	const int _height = input->inputAttributes[1];

	const int BmpRow = (3 * _width + 3) & ~3;
	byte* Bmp = (byte*) BxAlloc(4 + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + BmpRow * _height);
	Bmp[0] = 'B';
	Bmp[1] = 'x';
	Bmp[2] = 'B';
	Bmp[3] = 'M';
	...
	byte* BmpFocus = Bmp + 4 + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + BmpRow * (_height - 1);
	byte* StreamYBegin = input->out.streamY;
	byte* StreamUBegin = (input->nf == 3)? input->out.streamU : input->out.streamY;
	byte* StreamVBegin = (input->nf == 3)? input->out.streamV : input->out.streamY;
	for(int y = _height; y--; BmpFocus -= BmpRow)
	{
		byte* StreamY = StreamYBegin + (_height - y - 1) * input->extwidth;
		byte* StreamU = StreamUBegin + (_height - y - 1) * input->extwidth;
		byte* StreamV = StreamVBegin + (_height - y - 1) * input->extwidth;
		for(int x = 0; x + 2 < BmpRow; x += 3)
		{
			BmpFocus[x + 0] = *(StreamV++);
			BmpFocus[x + 1] = *(StreamU++);
			BmpFocus[x + 2] = *(StreamY++);
		}
	}
